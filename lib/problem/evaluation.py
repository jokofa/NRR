...
from lib.problem.formats import RPSolution
            ...
            if r and sum(r) > depot:    # not empty and not just depot idx
                ...
                for target in r[1:]:
                    try:
                        ...
                        if target > data.graph_size-1:
                            warnings.warn(f"index larger graph size encountered: {target} > {data.graph_size-1}")
                            target = 0
                            transit += np.linalg.norm(coords[source] - coords[target], ord=2)
                        else:
                            raise ie
                ...
                if cum_d > 1.0 + 2 * EPS:
                    warnings.warn(f"solution infeasible (demand = {cum_d}). setting cost and k to 'inf'")
                    cost = float("inf")
                    k = float("inf")
                    break
